...
@ To change the puzzle initial state:   To test the algorithm on different start states.
                                        In main function (end of file), all the 5 initial state are written in 3x3 matrix format.
...
puzzle size: 3 x 3, filled with numbers from [1, 8] the blank space 'b' is represented using '0'
Initial state: the initial 3x3 matrix values is given in main function
                (algorithms run on 5 different start state)
Goal state: the final goal is defined as a global variable --> [[1, 2, 3], [4, 5, 6], [7, 8, 0]]
...
class Puzzle:
    def __init__(self):                         # initialize all the variables
        self.node_data = [[0, 0, 0], [0, 0, 0], [0, 0, 0]]
        self.h_val = 0
        self.parent = None
        self.depth = 0

    """
    A 3x3 puzzle is created using initial state values
    """
    ...
    for algo in range(2):               # loop for 2 algorithms; 0 -> BFS and 1 -> A*
        ...
        for j in range(5):              # runs each algorithm for these 5 puzzles (initial states)
            if j == 0:
                ...
                initial_state = [[1, 2, 5], [0, 4, 3], [6, 8, 7]]
            elif j == 2:
                initial_state = [[0, 1, 2], [3, 4, 5], [6, 7, 8]]
            elif j == 3:
                initial_state = [[1, 2, 3], [4, 0, 5], [6, 7, 8]]
            else:
                initial_state = [[1, 3, 0], [4, 2, 5], [7, 8, 6]]
